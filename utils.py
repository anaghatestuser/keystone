    ...
    def _normalize_groups(self, identity_value):
        ...
        if 'name' in identity_value['groups']:
            ...
            def convert_json(group):
                if isinstance(group, str) and group.startswith('JSON:'):
                    try:
                        return jsonutils.loads(group.lstrip('JSON:'))
                    except ValueError:
                        # Malformed JSON group specification: treat it as an
                        # invalid group entry rather than letting the decode
                        # error escape as an unhandled exception.
                        return None
                return group

            group_dicts = [convert_json(g) for g in group_names_list]
            for g in group_dicts:
                # Each group entry must describe a group as a mapping with a
                # string name and a domain mapping, matching the shape the
                # plain-name branch below produces. The values originate from
                # attacker-influenced assertion data, so anything that does not
                # conform (a bare string/number, a nested list, malformed JSON,
                # a missing name, or a non-mapping domain) is rejected here
                # instead of being dereferenced further down the auth path.
                if (
                    not isinstance(g, dict)
                    or not isinstance(g.get('name'), str)
                    or not isinstance(g.get('domain'), dict)
                ):
                    msg = _(
            ...
            for name in group_names_list:
                # Group names are expected to be strings; reject anything else
                # (e.g. an int or dict from an attacker-controlled value that
                # literal-evals to a non-string element) as invalid input.
                if not isinstance(name, str):
                    msg = _(
                        "Invalid rule: %(identity_value)s. 'groups' must be a "
                        "list of group names."
